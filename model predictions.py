...
Original file is located at
    https://colab.research.google.com/drive/1OVTvAw9CFl15uUZmK-jmDMCbi7e2_wjO
...
df_raw = pd.read_csv(abspath_curr+'/data/data.csv',header=0)
## Making a copy of df_raw
df = df_raw.copy(deep=True)

## Getting the name of the target
target = 'bankrupt'

## Setting a temporary ID
df['id'] = np.arange(df.shape[0])

## Printing the dimensions of df
pd.DataFrame([[df.shape[0], df.shape[1]]], columns = ['# rows', '# columns'])

df.head()

bankrupt = pd.read_csv(abspath_curr+'/data/data.csv',header=0)
...
## Creating dictionary of paramaeter grids
param_grids = {}

# Parameter grid for LogisticRegression

## The parameter grid of tol
tol_grid = [10 ** -10, 10 ** -11, 10 ** -9] 

## The parameter grid of C
C_grid = [9, 8.99, 9.01, 8.95]
...
## The grids for alpha
alpha_grids = np.linspace(10**(-7),10**(-1),endpoint=True)

## The grids for learning_rate_init
learning_rate_init_grids = np.linspace(10**(-7),10**(-1),endpoint=True)
...
min_samples_split_grids = [1, 2, 10]

## The grids for min_samples_leaf
min_samples_leaf_grids = [1, 2, 10]

## Update param_grids
param_grids['rfc'] = [{'model__min_samples_split': min_samples_split_grids,
                       'model__min_samples_leaf': min_samples_leaf_grids}]

# Parameter for Histogram-based Gradient Boosting

## The grids for learning_rate
learning_rate_grids = [10 ** i for i in range(-7, 7)]
...
# Make directory for GridSearchCV results
